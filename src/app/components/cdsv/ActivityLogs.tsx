...
export function ActivityLogs() {
  ...
  }, [selectedFilter, searchQuery, ipFilter, userFilter, dateFrom, dateTo]);

  const selectedWarningLog = logsData.find(log => log.id === warningModalLogId) ?? null;
  ...
  const handleRowClick = (logId: string) => {
    const log = logsData.find(entry => entry.id === logId);
    if (!log) return;
  ...
  const exportFile = async (type: 'csv' | 'json') => {
    setIsExporting(true);
    try {
      const source = exportMode === 'all' ? logsData : filteredLogs;
          ...
          <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-4">
            <Card glass className="border border-[#10B981]/20">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-2xl font-bold text-[#E5E7EB]">{logsData.filter(l => l.status === 'success').length}</p><p className="text-xs text-[#9CA3AF]">Successful</p></div>
              </div>
            </Card>
            <Card glass className="border border-[#F59E0B]/20">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-2xl font-bold text-[#E5E7EB]">{logsData.filter(l => l.status === 'warning').length}</p><p className="text-xs text-[#9CA3AF]">Warnings</p></div>
              </div>
            </Card>
            <Card glass className="border border-[#EF4444]/20">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-2xl font-bold text-[#E5E7EB]">{logsData.filter(l => l.status === 'error').length}</p><p className="text-xs text-[#9CA3AF]">Errors</p></div>
              </div>
            </Card>
            <Card glass className="border border-[#3B82F6]/20">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-2xl font-bold text-[#E5E7EB]">{logsData.length}</p><p className="text-xs text-[#9CA3AF]">Total Events</p></div>
              </div>
            </Card>
          </div>
        ...
        <Card glass>
          ...
          {filteredLogs.length > 0 && (
            <div className="flex items-center justify-between border-t border-white/10 px-4 py-4">
              <p className="text-sm text-[#9CA3AF]">Showing {filteredLogs.length} of {logsData.length} entries</p>
